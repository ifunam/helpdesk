...
@user = User.new(:login => 'goz',:name =>" Gustavo Gomez Macias", :email => 'goz@fisica.unam.mx',:is_tech => true)
["Paginas web del instituto", "Otro"].each do |category_name|
  @user.categories << Category.new(:name => category_name)
end
@user.save

@user = User.new(:login => 'nglez',:name =>"Neptalí González Gómez", :email => 'nglez@fisica.unam.mx',:is_tech => true)
["Red inalámbrica", "Red cableada", "Soporte para windows y sus paquetes", "Telefonía"].each do |category_name|
  @user.categories << Category.new(:name => category_name)
end
@user.save

@user = User.new(:login => 'javo',:name =>"Javier Martínez ", :email => 'javo@fisica.unam.mx',:is_tech => true)
["Correo electrónico", "Virus", "Otro", "Soporte para windows y sus paquetes" ].each do |category_name|
  @user.categories << Category.new(:name => category_name)
end
@user.save

@user = User.new(:login => 'javier',:name =>"Javier Martínez ", :email => 'javier@fisica.unam.mx',:is_tech => true)
["Correo electrónico", "Virus", "Otro", "Soporte para windows y sus paquetes"].each do |category_name|
  @user.categories << Category.new(:name => category_name)
end
@user.save

@user = User.new(:login => 'dulce', :name =>"Dulce Maria Aguilar Tellez", :email => 'dulce@fisica.unam.mx',:is_tech => true)
["Fotografía y carteles", "Otro", "Soporte para windows y sus paquetes"].each do |category_name|
  @user.categories << Category.new(:name => category_name)
end
@user.save

@user = User.new(:login => 'garciag',:name =>"Alberto García", :email => 'garciag@fisica.unam.mx',:is_tech => true)
["Soporte para windows y sus paquetes", "Otro"].each do |category_name|
  @user.categories << Category.new(:name => category_name)
end
@user.save

@user = User.new(:login => 'alex', :password => 'alex', :name =>"Alejandro Juárez Robles", :email => 'alex@fisica.unam.mx',:is_tech => true)
...
@user = User.new(:login => 'natorro',:name =>"Carlos Ernesto López Natarén", :email => 'natorro@fisica.unam.mx',:is_tech => true)
